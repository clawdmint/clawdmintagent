...
export async function GET(request: NextRequest, { params }: RouteContext) {
  ...
  const relativeSvg = `/api/cpeg/${launch.tokenMint}/pegs/${pegId}/svg`;
  const relativePng = `/api/cpeg/${launch.tokenMint}/pegs/${pegId}/png`;
  const base = getBaseUrl(request);
  // Marketplaces and wallet preview surfaces (Solscan, Magic Eden, Phantom)
  // render PNG much more reliably than inline SVG, so we point the canonical
  // `image` field at our server-side PNG rasterizer. The deterministic SVG
  // source is still exposed via `properties.files` so anyone fetching the
  // metadata can pick the format they prefer.
  const metadata = {
    name,
    symbol: launch.symbol,
    description: `${name} is a deterministic cPEG identity generated by Clawdmint.`,
    image: `${base}${relativePng}`,
    external_url: `${base}/${launch.tokenMint}#peg-${pegId}`,
    attributes: Object.entries(traits).map(([trait_type, value]) => ({ trait_type, value })),
    properties: {
      category: "image",
      files: [
        { uri: `${base}${relativePng}`, type: "image/png" },
        { uri: `${base}${relativeSvg}`, type: "image/svg+xml" },
      ],
    },
  };
