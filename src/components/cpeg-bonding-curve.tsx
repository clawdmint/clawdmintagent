...
export function CpegBondingCurve({ tokenMint }: CpegBondingCurveProps) {
  ...
  return (
    <section className="border border-[#53c7ff]/25 bg-gradient-to-br from-[#06101a] via-[#070d18] to-[#040814] p-5 md:p-8">
      <CurveHeader cluster={derived?.cluster || ""} loading={loading} error={error} />

      <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1.65fr)_minmax(0,1fr)]">
...
function CurveHeader({ cluster, loading, error }: { cluster: string; loading: boolean; error: string | null }) {
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div>
        ...
        <p className="mt-2 text-2xl font-black uppercase tracking-tight text-white md:text-3xl">
          Backing reservoir <span className="text-[#53c7ff]">/ live</span>
        </p>
...
function HourlyHistogram({ activity }: { activity: ActivityResponse | null }) {
  ...
  return (
    <div className="border border-white/10 bg-black/40 p-4 md:p-5">
          ...
          hourly.map((bucket, idx) => {
            ...
            return (
              <div
                ...
                title={`${new Date(bucket.hour_ts).toLocaleString()}\nmint ${bucket.mints}, burn ${bucket.burns}`}
              >
                <div
                  className="w-full bg-[#53ffac]/85 shadow-[0_0_8px_#53ffac55]"
                  style={{ height: `${mintH}%`, minHeight: bucket.mints > 0 ? 2 : 0 }}
                />
                <div
                  className="mt-[1px] w-full bg-[#ec5cff]/85 shadow-[0_0_8px_#ec5cff55]"
                  style={{ height: `${burnH}%`, minHeight: bucket.burns > 0 ? 2 : 0 }}
                />
              </div>
            );
          })
...
function CumulativeSparkline({
  ...
  const points = useMemo(() => {
    ...
    return daily
      ...
      .join(" ");
  }, [daily, innerH, innerW, maxCumulative, minCumulative, padX, padY]);
